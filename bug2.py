...
def create_square_obstacles(o_x, o_y):
...
def create_circular_obstacles(o_x, o_y):
    """ Create circular obstacles """
    for i in range(15, 40):
        for j in range(5, 30):
            if np.sqrt((i - 25)**2 + (j - 5)**2) < 10:
                o_x.append(i)
                o_y.append(j)

    for i in range(120, 150):
        for j in range(40, 70):
            if np.sqrt((i - 130)**2 + (j - 50)**2) < 20:
                o_x.append(i)
                o_y.append(j)
    
    # """ Generate circular obstacles given a center and radius. """
    # for i in range(center_x - radius, center_x + radius):
    #     for j in range(center_y - radius, center_y + radius):
    #         # Check if the point (i, j) is within the circle's radius
    #         if (i - center_x) ** 2 + (j - center_y) ** 2 <= radius ** 2:
    #             obs_x.append(i)
    #             obs_y.append(j)

def create_line_obstacles(o_x, o_y):
    """ Create line obstacles """
...
def main():
   ...
   # Create different shapes of obstacles
    create_square_obstacles(o_x, o_y)     # Square obstacles
    create_circular_obstacles(o_x, o_y)   # Circular obstacles
    create_line_obstacles(o_x, o_y)       # Line obstacles
